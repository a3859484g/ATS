...
{
    public class PIOFlow_ATMS_Charging : PIOFlow_Charging
    {
        private string rfPIOStart = "PIO-out00";
        ...
        private float rfPIOOpen = 0;
        private float rfPIOClose = 1;
        ...
        public override void Initial(AlarmHandler alarmHandler, MIPCControlHandler mipcControl, string pioName, string pioDirection, string normalLogName)
        {
            ...
            rfPIOStart = String.Concat(rfPIOStart, pioDirection);
            ...
            allIOData.Add(rfPIOStart, new DataDelayAndChange(delayTime, EnumDelayType.OnDelay));
        ...
        private void CheckChargingA()
        {
            ...
            //if ((Math.Abs(localData.BatteryInfo.Battery_A) + Math.Abs(localData.BatteryInfo.Meter_A)) > localData.BatteryConfig.ChargingMaxA)
            //{
        ...
        public void Check溫度()
        {
            //if ((localData.BatteryInfo.Battery_溫度1 > 0 && localData.BatteryInfo.Battery_溫度1 > localData.BatteryConfig.ChargingMaxTemp) ||
            //    (localData.BatteryInfo.Battery_溫度2 > 0 && localData.BatteryInfo.Battery_溫度2 > localData.BatteryConfig.ChargingMaxTemp))
            //{
            //    SetAlarmCode(EnumLoadUnloadControlErrorCode.ChargingWith_SensorEMO);
            //    ChargingStep = EnumChargingStatus.SendStopCharging;
            //}
        ...
        private void ChargingThread()
        {
            ...
            try
            {
                ...
                if (GetConfirmSensorOnOff)
                {
                    ...
                    {
                        while (changeChannelThread != null && changeChannelThread.IsAlive)
                            Thread.Sleep(100);

                        ChangeRFPIOChannelByAddressID(chargerStationAddressID);

                        while (changeChannelThread != null && changeChannelThread.IsAlive)
                            Thread.Sleep(100);


                        if (nowChannelID == localData.TheMapInfo.AllAddress[chargerStationAddressID].RFPIOChannelID &&
                            nowDeviceID == localData.TheMapInfo.AllAddress[chargerStationAddressID].RFPIODeviceID)
                        {
                        ...
                        {
                            SetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerError_RFPIO切換失敗);
                        }
                    }
                    else
                    {
                        SetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerError_AGV不在充電站或迷航中);
                    }
                }
                else
                {
        ...
        public override void AlarmCodeClear()
        {
            ...
            ResetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerError_AGV不在充電站或迷航中);
            ResetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerError_RFPIO切換失敗);
        }
        ...
        public override void PIOTestOnOff(string tag, bool onOff)
        {
            if (localData.AutoManual == EnumAutoState.Manual && !Charging)
            {
                ...
                {
                    if (onOff)
                        mipcControl.SendMIPCDataByMIPCTagName(new List<string>() { rfPIOStart, tag }, new List<float>() { rfPIOOpen, 1 });
                    else
                        mipcControl.SendMIPCDataByMIPCTagName(new List<string>() { rfPIOStart, tag }, new List<float>() { rfPIOClose, 0 });
                }
            }
        }
        ...
        public override void ChangeRFPIOChannelByAddressID(string addressID)
        {
            ...
            {
                if (true) //  FRPIO Comport/Internat Connected
                {
                    if (changeChannelThread == null || !changeChannelThread.IsAlive)
                    {
                        if (nowChannelID != localData.TheMapInfo.AllAddress[addressID].RFPIOChannelID ||
                            nowDeviceID != localData.TheMapInfo.AllAddress[addressID].RFPIODeviceID)
                        {
                            changeChannelID = localData.TheMapInfo.AllAddress[addressID].RFPIOChannelID;
                            changeDeviceID = localData.TheMapInfo.AllAddress[addressID].RFPIODeviceID;
                            changeChannelThread = new Thread(ChangeChannelThread);
                            changeChannelThread.Start();
                        }
                    }
                }
            }
        ...
        public override void ChangeRFPIOChannel(string deviceID, string channelID)
        {
            if (true) //  FRPIO Comport/Internat Connected
            {
                if (changeChannelThread == null || !changeChannelThread.IsAlive)
                {
                    if (nowChannelID != channelID || nowDeviceID != deviceID)
                    {
        ...
        private void ChangeChannelThread()
        {
            // 這邊log自己加一加.
            // try change to changeChanel
            // 結果??
            // 成功的話 
            nowChannelID = changeChannelID;
            nowDeviceID = changeDeviceID;
            // 失敗 且是因為斷線
            //     Conneted = false
            //     SetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerRFPIO_連線失敗);
        }
        #endregion

        private void ConnectRFPIO()
        {
            //if (!connected)
            //{
            //    if (connect Scuess)
            //        ResetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerRFPIO_連線失敗);
            //    else
            //        SetAlarmCode(EnumLoadUnloadControlErrorCode.ChargerRFPIO_連線失敗);
            //}
        }
